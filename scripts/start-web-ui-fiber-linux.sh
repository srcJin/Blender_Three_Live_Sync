...
# Blender Three Live Sync - React Three Fiber Client Startup Script (Linux)
# This script starts the React Three Fiber web client with full interaction features

echo "🎨 Starting Blender Three Live Sync React Three Fiber Client (Linux)..."
echo "📍 Location: $(pwd)"
echo ""
...
# Check if the web_ui_fiber directory exists
if [ ! -d "web_ui_fiber" ]; then
    echo "❌ Error: web_ui_fiber directory not found"
...
if [ ! -f "package.json" ]; then
    echo "❌ Error: package.json not found in web_ui_fiber directory"
    exit 1
fi
...
echo "🌐 Starting React Three Fiber development server..."
